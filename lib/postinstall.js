...
function getDownloadInfo(platformInfo, version) {
  ...
  if (isWindows) {
    ...
    // The version tag includes git hash (e.g., 2025.11.15+hash) but filenames only use the date part
    ...
    if (isMac) {
      // macOS format: uctags-{version}-macos-{os_version}-{arch}.release.tar.gz
      // Use macOS 12.0 for compatibility
      platformName = 'macos-12.0';
      archName = arch === 'x86_64' ? 'x86_64' : 'arm64';
    } else if (isLinux) {
      // Linux format: uctags-{version}-linux-{arch}.release.tar.gz
      platformName = 'linux';
      archName = arch;
    } else {
    ...
    const filename = isMac
      ? `uctags-${versionDate}-${platformName}-${archName}.release.tar.gz`
      : `uctags-${versionDate}-${platformName}-${archName}.release.tar.gz`;

    return {
      downloadUrl: `https://github.com/universal-ctags/ctags-nightly-build/releases/download/${version}/${filename}`,
